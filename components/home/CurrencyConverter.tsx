...
export function CurrencyConverter() {
  const { t } = useLang();
  const { rates, ratesLive } = useCurrency();
  const [amount, setAmount] = React.useState(1000);
  const [from, setFrom] = React.useState<Currency>('USD');
  const [to, setTo] = React.useState<Currency>('TRY');

  // rates context değiştiğinde (canlı kur gelince) yeniden hesaplanır.
      ...
      <div className="rounded-2xl sm:rounded-3xl border border-gold-400/30 bg-[color:var(--bg-card)] shadow-xl shadow-black/10 p-5 sm:p-7">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5 pb-4 mb-5 border-b border-[color:var(--border)]">
          <div className="flex items-center gap-2 flex-wrap">
            <Badge variant="gold"><RefreshCw size={11} /> {ratesLive ? t('currency.live') : t('currency.indicative')}</Badge>
            <span className="text-xs text-[color:var(--fg-muted)]">{t('currency.source')}</span>
          </div>
          <div className="flex items-center gap-1.5 text-[11px] font-medium text-[color:var(--fg-muted)] tabular-nums">
            <span className="rounded-md border border-[color:var(--border)] bg-[color:var(--bg-elev)] px-2 py-1">1 USD = ₺{rates.TRY.toFixed(2)}</span>
            <span className="rounded-md border border-[color:var(--border)] bg-[color:var(--bg-elev)] px-2 py-1">₼{rates.AZN.toFixed(2)}</span>
            <span className="rounded-md border border-[color:var(--border)] bg-[color:var(--bg-elev)] px-2 py-1">€{rates.EUR.toFixed(2)}</span>
          </div>
        </div>

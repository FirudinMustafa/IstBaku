...
export function CountryGuidesClient({ initial }: { initial: Guide[] }) {
  ...
  async function save() {
    ...
    if (res.ok) {
      ...
      setOpen(false);
      setEditing(null);
      toast({ variant: 'success', title: exists ? t('admin.guides.toast.updated') : t('admin.guides.toast.added') });
      router.refresh();
    } else {
      toast({ variant: 'error', title: t('admin.guides.toast.saveFail'), description: res.error });
    }
  }

  async function remove(iso: string) {
    if (!confirm(t('admin.guides.confirmDelete').replace('{iso}', iso))) return;
    const res = await deleteGuideAction(iso);
    if (res.ok) {
      setGuides((cur) => cur.filter((g) => g.iso !== iso));
      toast({ variant: 'info', title: t('admin.guides.toast.deleted') });
      router.refresh();
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">{t('admin.guides.title')}</h1>
          <p className="text-sm text-[color:var(--fg-muted)] mt-1">{t('admin.guides.subtitle')}</p>
        </div>
        <Button variant="gold" onClick={startCreate}><Plus size={14} /> {t('admin.guides.newGuide')}</Button>
      </div>
        ...
        {guides.map((g) => (
          <Card key={g.iso}>
            <CardBody>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-[color:var(--fg-muted)] flex items-center gap-2 mt-0.5 flex-wrap">
                    ...
                    <span>{g.pages} {t('admin.guides.pages')}</span>
                    <span>·</span>
              ...
              <div className="mt-3 text-[10px] text-[color:var(--fg-faint)]">{t('admin.guides.updated').replace('{date}', g.updatedAt)}</div>
              <div className="mt-4 grid grid-cols-3 gap-1.5">
                ...
                <Button variant="outline" size="sm" onClick={() => startEdit(g)}>{t('common.edit')}</Button>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing && guides.find((g) => g.iso === editing.iso) ? t('admin.guides.editDialog') : t('admin.guides.newGuide')} size="lg">
        {editing && (
          <div className="space-y-3">
            <div className="grid sm:grid-cols-2 gap-3">
              <div>
                <Label>{t('admin.guides.fCountry')}</Label>
                <Select
                  value={editing.iso}
                  onChange={(e) => {
              ...
              <div>
                <Label>{t('admin.guides.fLanguage')}</Label>
            ...
            <div className="rounded-xl border-2 border-dashed p-5 text-center bg-[color:var(--bg-elev)]">
              ...
              <p className="text-sm mt-2 text-[color:var(--fg-muted)]">{t('admin.guides.pdfUpload')}</p>
              <Button type="button" variant="outline" size="sm" className="mt-2 gap-1.5">
                <Upload size={13} /> {t('admin.guides.pickFile')}
              </Button>
            </div>

...
export function RpaAdminClient({ pending, sentCount }: { pending: AdminRpaReport[]; sentCount: number }) {
  ...
  async function markSent(id: string) {
    ...
    if (res.ok) {
      ...
      toast({ variant: 'success', title: 'Rapor gönderildi olarak işaretlendi', description: 'Kullanıcıya bildirim + mail gitti.' });
      router.refresh();
    } else {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <h1 className="text-xl font-bold inline-flex items-center gap-2"><FileText size={20} className="text-gold-300" /> RPA Rapor Talepleri</h1>
        <div className="flex items-center gap-2 text-sm">
          <Badge variant="gold">{rows.length} bekliyor</Badge>
          <Badge variant="success">{sentCount} gönderildi</Badge>
        </div>
      </div>

      {rows.length === 0 ? (
        <Card><CardBody className="text-center py-12 text-[color:var(--fg-muted)]">Bekleyen RPA talebi yok.</CardBody></Card>
      ) : (
            ...
            <Card key={r.id}>
              <CardBody className="space-y-3">
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  ...
                  <Badge variant="gold">bekliyor</Badge>
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  <Input
                    value={fileUrls[r.id] ?? ''}
                    onChange={(e) => setFileUrls((s) => ({ ...s, [r.id]: e.target.value }))}
                    placeholder="Rapor dosya linki (opsiyonel)"
                    className="flex-1 min-w-[200px]"
                  />
                  <Button variant="gold" size="sm" onClick={() => markSent(r.id)} loading={busy === r.id}>
                    <Send size={14} /> Gönderildi işaretle
                  </Button>
                </div>
                <p className="text-[11px] text-[color:var(--fg-faint)] inline-flex items-center gap-1">
                  <Mail size={11} /> Raporu elle hazırlayıp kullanıcıya mailleyin, sonra burada gönderildi olarak işaretleyin.
                </p>
              </CardBody>
            </Card>
          ))}

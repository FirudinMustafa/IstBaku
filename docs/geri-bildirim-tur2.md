...
- **2026-06-11 (commit bfbaa88):** ✅ **D1** ilan düzenlemede 'İlana dön' + 'İlan Düzenle' overlap düzeltildi (inline-flex→flex w-fit) — objektif denetim: 11 public sayfa × 2 görünüm yatay taşma YOK. ✅ **H3** lightbox komşu-foto preload (geçiş kasması). ✅ **C1 batch** galeri/arama/düzenleme görünür metinleri 6 dile. ✅ **G2/E1 zaten yapılmış** doğrulandı (filtre eşyalı+site ayrı+site adı; KYC ülkeye göre ofis alanları). **C1 kalan:** dağınık düşük-trafik hardcoded (bazı aria-label/bileşen) — süren iş.
- **2026-06-11 (commit 5df8de9) — M3 BİTTİ:** blog yazıcı başvuru→onay akışı.
- **2026-06-10 (commit 0b25088) — "senin aksiyonların":** ✅ **BLOB token DOĞRULANDI** — canlı office-docs upload endpoint'ine gerçek JPEG POST → HTTP 200, Vercel Blob URL döndü + URL erişilebilir (image/jpeg). Yani belge yükleme prod'da ÇALIŞIYOR, token bağlı. ✅ **A1 backfill** — yerelde prod BLOB token olmadığından doğrudan çalıştıramıyorum; bunun yerine `admin/listings`'e **tek-tık 'Mevcut ilanları damgala'** butonu + `backfillWatermarkAction` (8'erli idempotent, Vercel'de token'la) eklendi (28 onaylı ilan bekliyor). ⚠️ **AZ tam mahalle:** açık veri yalnız şehir/rayon seviyesinde; TR'deki gibi məhəllə-seviyesi açık veri YOK → AZ küratörlü kalır.
- **2026-06-10 (commit e4ee59e) — G1 BİTTİ:** Tam TR+AZ mahalle veri seti. `neighborhoods` tablosu + migration 0015 + `/api/neighborhoods` + `useNeighborhoods` hook; FilterSidebar + NewListingClient datalist'leri bağlandı. Seed: **73.381 kayıt** (TR ~73.305 `turkey-neighbourhoods` paketinden + AZ küratörlü) Neon prod'a uygulandı. **Canlıda doğrulandı:** İstanbul/Kadıköy 21 mahalle temiz dönüyor. App il/ilçe adları seed ile eşleşiyor. Not: TR tam; AZ şu an küratörlü (Bakü+büyük şehirler) — tam AZ açık-veri kısıtlı, ayrı kaynak gerektirir.
- **2026-06-10 (commit 9ea1591):** ✅ **H2 harita** — fitBounds yalnız ilk çizimde (hover/filtrede zıplama yok); hover'da yalnız ilgili 2 marker güncellenir (canlıda 26 marker çalışıyor) · ✅ **H1 homepage** — 5× blur-3xl (ağır GPU blur) kaldırıldı, Safari/Mac scroll maliyeti düştü (radial gradient yumuşaklığı korundu). K1 PDF + I1 geri-buton **canlıda doğrulandı** (PDF 295KB indi, geri-buton bindirme yok). Build temiz.
  - **KALAN TEK BÜYÜK İŞ: G1** tam TR+AZ mahalle veri seti → DB tablo + `/api/neighborhoods` + filtre/wizard datalist. Ayrı odaklı tur (sourcing + seed + UI).
- **2026-06-10 (commit 38394cb):** ✅ **K1** raporlar gerçek PDF (jspdf+html2canvas, sayfalı, logo başlık) — window.print kaldırıldı · ✅ **I1** geri-buton boşluğu (-mb-1→pt-4 pb-2, başlığa bindirme giderildi) · ✅ **K2** demo formu fokus kaybı I4 (FocusTrap) ile çözülmüş (Modal FocusTrap kullanıyor). Build temiz.
  - **Sahip kararları:** G1=tam TR+AZ mahalle veri seti import · I1=başlık bindirme (yapıldı) · H=tüm alanlar (harita+scroll+galeri+Mac/Safari) iyileştir · K1=gerçek PDF (yapıldı).
  - **Sıradaki büyük işler:** **G1** (tam mahalle veri seti → DB tablo + API + filtre/wizard; ağ erişimi var, sourcing+storage adımı) · **H** kapsamlı perf turu.
- **2026-06-10 (commit 1990864):** ✅ **L1** kart no 4'lü gruplama · ✅ **L2** son kullanma MM/YY otomatik '/' · ✅ **J3** ApprovedBadge altın gradyan+ring (belirgin) · ✅ **I2** ofis '(N yorum)' → yorumlara kaydırır · ✅ **I3** aktif nav linkine tekrar basınca en başa kaydır+refresh. Build temiz.
